...
-- When right arrow is touched, move dog right
...
local function up (touch)
    if (dog ~= nil) then
        dog:setLinearVelocity( 0, LINEAR_VELOCITY )
    end
end
...
-- Move dog horizontally
local function movePlayer (event)
    dog.x = dog.x + motionx
end
 
-- Stop dog movement when no arrow is pushed
...
local function ReplaceDog ()
    dog = display.newImageRect("Images/dog.png", 100, 150)
    dog.x = 150 
    dog.y = 680
    dog.width = 110
    dog.height = 110
    ...
    -- intialize horizontal movement of dog 
    motionx = 0

    -- add physics body
    physics.addBody( dog, "dynamic", { density=0, friction=0.5, bounce=0, rotation=0,} )

    -- prevent dog from being able to tip over
    dog.isFixedRotation = true
...
local function MakeBonesVisible()
    bone1.isVisible = true
    bone2.isVisible = true
    bone3.isVisible = true 
end
    ...
    if ( event.phase == "began" ) then
        ...
        if  (event.target.myName == "fence1") or 
            ...
            if (numLives == 2) then
                ...
                heart2.isVisible = true
                heart3.isVisible = true
                timer.performWithDelay(200, ReplaceDog) 
        ...
        if  (event.target.myName == "bone1") or
            (event.target.myName == "bone2") or
            (event.target.myName == "bone3") then

            -- get the bone that the user hit
            theBone = event.target

            -- stop the dog from moving
            motionx = 0

            -- make the dog invisible
            dog.isVisible = false
...
local function AddCollisionListeners()
    -- if dog collides with bone, onCollision will be called  
    ...
    bone1.collision = onCollision
    bone1:addEventListener( "collision" )
    bone2.collision = onCollision
    bone2:addEventListener( "collision" )
    bone3.collision = onCollision
    bone3:addEventListener( "collision" )
...
local function RemoveCollisionListeners()
    ...
    bone1:removeEventListener( "collision" )
    bone2:removeEventListener( "collision" )
    bone3:removeEventListener( "collision" )
...
local function AddPhysicsBodies()
    ...
    physics.addBody(bone2, "static",  {density=0, friction=0, bounce=0} )
    physics.addBody(bone3, "static",  {density=0, friction=0, bounce=0} )
...
function ResumeGame()
    ...
    -- make dog visible again
    dog .isVisible = true
    
    if (questionsAnswered > 0) then
        if (theBone ~= nil) and (theBone.isBodyActive == true) then
            physics.removeBody(theBone)
            theBone.isVisible = false
        end
    end
...
function scene:create( event )
    ...
    -- Insert objects into the scene group in order to ONLY be associated with this scene
    sceneGroup:insert( bone1 )

    --bone2
    bone2 = display.newImageRect ("Images/bone.png", 70, 70)
    bone2.x = 230
    bone2.y = 480
    bone2.myName = "bone2"

    -- Insert objects into the scene group in order to ONLY be associated with this scene
    sceneGroup:insert( bone2 )

    --bone3
    bone3 = display.newImageRect ("Images/bone.png", 70, 70)
    bone3.x = display.contentWidth * 0.5 / 8
    bone3.y = display.contentHeight  * .98 / 3
    bone3.myName = "bone3"

    -- Insert objects into the scene group in order to ONLY be associated with this scene
    sceneGroup:insert( bone3 )
...
function scene:show( event )
    ...
    elseif ( phase == "did" ) then
        ...
        -- create the dog , add physics bodies and runtime listeners
        ReplaceDog ()
